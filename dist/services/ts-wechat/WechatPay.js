...
class WechatPay {
    ...
    payToOne(order) {
        return new Promise((resolve, reject) => {
            ...
            order.nonce_str = order.nonce_str ? order.nonce_str : WechatTool_1.default.getRadomStr();
    ...
    async getBrandWCPayRequestParams(order) {
        return new Promise((resolve, reject) => {
            let options = {
                ...
                nonce_str: WechatTool_1.default.getRadomStr(),
                body: order.body,
                openid: order.openid,
                out_trade_no: order.out_trade_no || new Date().getTime().toString(),
                total_fee: order.total_fee,
                spbill_create_ip: order.spbill_create_ip || WechatTool_1.default.getSelfIp(),
                notify_url: order.notify_url || this.wechatConfig.notify_url,
                trade_type: order.trade_type || 'JSAPI'
            };
            let sign = WechatTool_1.default.sign(options, this.wechatConfig.apiKey);
            options['sign'] = sign;
            let xml = WechatTool_1.default.bulildXml({ xml: options });
            console.log(xml);
            let parsed_url = url.parse(WechatTool_1.default.urls.PAY.UNIFIED_ORDER);
            let req = https.request({
                ...
                res.on('end', async () => {
                    let jsonStr = await WechatTool_1.default.parseXml(content);
                    console.log(jsonStr);
                    ...
                    let paySign = WechatTool_1.default.sign(payargs, this.wechatConfig.apiKey);
