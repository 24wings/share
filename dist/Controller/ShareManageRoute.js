...
let ShareManageRoute = class ShareManageRoute extends lib_1.Core.Route.BaseRoute {
    ...
    async taskTagDelete() {
        let action = await this.service.db.taskTagModel.findByIdAndRemove(this.req.query._id).exec();
        this.res.redirect(`/share-admin/taskTag-list`);
    }
    async taskDelete() {
        var _id = this.req.query._id;
...
ShareManageRoute = __decorate([
    lib_1.Core.Route.Controller({
        service: 'share-manage',
        viewPath: 'share-mange'
    })
], ShareManageRoute);
exports.default = ShareManageRoute;
